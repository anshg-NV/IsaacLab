...
parser.add_argument("--num_rollouts", type=int, default=25, help="Number of rollouts.")
...
def rollout(policy, env, success_term, horizon, device):
    ...
    # diffusion policy expects (B, T, *obs_shape) per key.
    # maintain frame-stack deque to achieve that
    # BC uses frame_stack = 1
    try:
        frame_stack = max(1, int(policy.policy.global_config.train.frame_stack))
    ...
    for i in range(horizon):
        ...
        # robomimic v0.5's process_obs expects HWC uint8 for RGB and rescales internally, so we only strip the batch dim.
        ...
        if frame_stack > 1:
            if obs_history is None:
                obs_history = {
                    ...
                    for k, v in obs.items() if isinstance(v, torch.Tensor)
                }
            else:
                for k in obs_history:
                    obs_history[k].append(obs[k].unsqueeze(0))
            policy_obs = {k: torch.cat(list(q), dim=0) for k, q in obs_history.items()}
        else:
            policy_obs = obs
